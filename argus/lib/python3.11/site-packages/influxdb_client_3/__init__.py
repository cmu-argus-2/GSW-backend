...
from influxdb_client_3.write_client import InfluxDBClient as _InfluxDBClient
from influxdb_client_3.write_client import Point, WriteOptions
...
from influxdb_client_3.write_client.client.write_api import (ASYNCHRONOUS,
                                                             SYNCHRONOUS,
                                                             PointSettings)
from influxdb_client_3.write_client.client.write_api import \
    WriteApi as _WriteApi
from influxdb_client_3.write_client.domain.write_precision import \
    WritePrecision
...
def _merge_options(defaults, exclude_keys=None, custom=None):
    ...
    return _deep_merge(
        defaults,
        {key: value for key, value in custom.items() if key not in exclude_keys},
    )
...
class InfluxDBClient3:
    def __init__(
        ...
        **kwargs,
        ...
        self._write_client_options = (
            ...
            if write_client_options is not None
        ...
        self._write_api = _WriteApi(
            influxdb_client=self._client, **self._write_client_options
        )
        ...
        self._flight_client = FlightClient(
            f"grpc+tls://{hostname}:{port}", **self._flight_client_options
        )
    ...
    def _process_dataframe(
        self, df, measurement_name, tag_columns, timestamp_column, database, **kwargs
    ):
        ...
        if measurement_name is None:
            measurement_column = next(
                (
                    col
                    for col in ["measurement", "iox::measurement"]
            ...
            if measurement_column:
                for measurement in df[measurement_column].unique():
                    df_measurement = df[df[measurement_column] == measurement].drop(
                        columns=[measurement_column]
                    )
                    self._write_api.write(
                        bucket=self._database,
                        record=df_measurement,
    ...
    def query(
        ...
        **kwargs,
        ...
        try:
            ...
            opts = _merge_options(
                optargs, exclude_keys=["query_parameters"], custom=kwargs
            )
