...
from influxdb_client_3.write_client.client.write.point import (
    _ESCAPE_KEY, _ESCAPE_MEASUREMENT, _ESCAPE_STRING, DEFAULT_WRITE_PRECISION)
...
class DataframeSerializer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        data_frame_timestamp = (
            ...
            else data_frame[timestamp_column]
        )
        ...
        if timestamp_timezone:
            if isinstance(data_frame_timestamp, pd.DatetimeIndex):
                data_frame_timestamp = data_frame_timestamp.tz_localize(
                    timestamp_timezone
                )
            else:
                data_frame_timestamp = data_frame_timestamp.dt.tz_localize(
                    timestamp_timezone
                )

        if (
            hasattr(data_frame_timestamp, "tzinfo")
            and data_frame_timestamp.tzinfo is None
        ):
        ...
        for index, (key, value) in columns:
            ...
            if (
                issubclass(value.type, np.integer)
    ...
    def serialize(self, chunk_idx: int = None):
        ...
        if chunk_idx is None:
            ...
            logger.debug(
                "Serialize chunk %s/%s ...", chunk_idx + 1, self.number_of_chunks
            )
            chunk = self.data_frame[
        ...
        if self.first_field_maybe_null:
            ...
            lp = (
                re.sub("^(( |[^ ])* ),([a-zA-Z0-9])(.*)", "\\1\\3\\4", self.f(p))
                for p in filter(
                    lambda x: _any_not_nan(x, self.field_indexes), _itertuples(chunk)
                )
            )
            return list(lp)
        else:
...
class PolarsDataframeSerializer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.column_indices = {
            name: index for index, name in enumerate(data_frame.columns)
        }

        if (
            self.timestamp_column is None
            or self.timestamp_column not in self.column_indices
        ):
            raise ValueError(
    ...
    def to_line_protocol(self, row):
        ...
        tags = ",".join(
            ...
            if row[self.column_indices[col]] is not None
            and row[self.column_indices[col]] != ""
        )
        ...
        fields = ",".join(
            f'{col}="{self.escape_value(row[self.column_indices[col]])}"'
            if isinstance(row[self.column_indices[col]], str)
            else f"{col}={str(row[self.column_indices[col]]).lower()}"
            if isinstance(row[self.column_indices[col]], bool)  # Check for bool first
            else f"{col}={row[self.column_indices[col]]}i"
            if isinstance(row[self.column_indices[col]], int)
            else f"{col}={row[self.column_indices[col]]}"
            for col in self.column_indices
            if col not in self.tag_columns + [self.timestamp_column]
            and row[self.column_indices[col]] is not None
            and row[self.column_indices[col]] != ""
        )
        ...
        if df[self.timestamp_column].dtype in [pl.Int32, pl.Int64]:
            ...
            if self.precision in [None, "ns"]:
                df = df.with_columns(
                    ...
                    .alias(self.timestamp_column)
                )
            elif self.precision == "us":
                df = df.with_columns(
                    pl.col(self.timestamp_column)
                    .dt.epoch(time_unit="us")
                    .alias(self.timestamp_column)
                )
...
def data_frame_to_list_of_points(
    data_frame, point_settings, precision=DEFAULT_WRITE_PRECISION, **kwargs
):
    """
    Serialize DataFrame into LineProtocols.
    ...
    return DataframeSerializer(
        data_frame, point_settings, precision, **kwargs
    ).serialize()


def polars_data_frame_to_list_of_points(
    data_frame, point_settings, precision=DEFAULT_WRITE_PRECISION, **kwargs
):
    """
    Serialize DataFrame into LineProtocols.
    ...
    :key data_frame_timestamp_timezone: name of the timezone which is used for timestamp column - ``DataFrame``
    """  # noqa: E501
    return PolarsDataframeSerializer(
        data_frame, point_settings, precision, **kwargs
    ).serialize()
