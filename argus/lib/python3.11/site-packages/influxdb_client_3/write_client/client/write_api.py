...
from influxdb_client_3.write_client.client._base import (_HAS_DATACLASS,
                                                         _BaseWriteApi)
from influxdb_client_3.write_client.client.util.helpers import \
    get_org_query_param
from influxdb_client_3.write_client.client.write.dataframe_serializer import (
    DataframeSerializer, PolarsDataframeSerializer)
from influxdb_client_3.write_client.client.write.point import (
    DEFAULT_WRITE_PRECISION, Point)
...
class _BatchItemKey(object):
    ...
    def __str__(self) -> str:
        return "_BatchItemKey[bucket:'{}', org:'{}', precision:'{}']".format(
            str(self.bucket), str(self.org), str(self.precision)
        )
...
class _BatchItem(object):
    ...
    def __str__(self) -> str:
        return "_BatchItem[key:'{}', size: '{}']".format(str(self.key), str(self.size))
...
class _BatchResponse(object):
    ...
    def __str__(self) -> str:
        return "_BatchResponse[status:'{}', '{}']".format(
            "failed" if self.exception else "success", str(self.data)
        )
        ...
        **kwargs,
        ...
        if self._write_options.write_type is WriteType.batching:
            ...
            self._disposable = self._subject.pipe(
                    ...
                    lambda window: window.pipe(
                        ...
                        ops.map(
                            lambda group: group.pipe(
                                ops.to_iterable(),
                                ops.map(
                                    lambda xs: _BatchItem(
                                        key=group.key,
                                        data=_body_reduce(xs),
                                        size=len(xs),
                                    )
                                ),
                            )
                ...
                ops.map(
                    mapper=lambda batch: self._to_response(
                        data=batch, delay=self._jitter_delay()
                    )
                ),
                ops.merge_all(),
...
- https://influxdb-client.readthedocs.io/en/stable/usage.html#how-to-use-asyncio
    ...
    def write(
        ...
        **kwargs,
        ...
        if self._write_options.write_type is WriteType.batching:
            return self._write_batching(bucket, org, record, write_precision, **kwargs)

        payloads = defaultdict(list)
        self._serialize(record, write_precision, payloads, **kwargs)

        _async_req = (
            True if self._write_options.write_type == WriteType.asynchronous else False
        )
    ...
    def _write_batching(
        self, bucket, org, data, precision=DEFAULT_WRITE_PRECISION, **kwargs
    ):
        if isinstance(data, bytes):
            ...
            self._write_batching(
                bucket, org, data.encode(_UTF_8_encoding), precision, **kwargs
            )

        elif isinstance(data, Point):
            self._write_batching(
                bucket, org, data.to_line_protocol(), data.write_precision, **kwargs
            )
            ...
            self._write_batching(
                bucket,
                org,
                Point.from_dict(data, write_precision=precision, **kwargs),
                precision,
                **kwargs,
            )

        elif "polars" in str(type(data)):
            serializer = PolarsDataframeSerializer(
                data,
                self._point_settings,
                precision,
                self._write_options.batch_size,
                **kwargs,
            )
            for chunk_idx in range(serializer.number_of_chunks):
                self._write_batching(
                    bucket, org, serializer.serialize(chunk_idx), precision, **kwargs
                )
            ...
            self._write_batching(
                bucket, org, dataclasses.asdict(data), precision, **kwargs
            )
            ...
            data.subscribe(
                lambda it: self._write_batching(bucket, org, it, precision, **kwargs)
            )
            pass
    ...
    def _http(self, batch_item: _BatchItem):
        ...
        if self._retry_callback:
            ...
            def _retry_callback_delegate(exception):
                return self._retry_callback(
                    batch_item.to_key_tuple(), batch_item.data, exception
                )
        ...
        retry = self._write_options.to_retry_strategy(
            retry_callback=_retry_callback_delegate
        )

        self._post_write(
            False,
            batch_item.key.bucket,
            batch_item.key.org,
            batch_item.data,
            batch_item.key.precision,
            urlopen_kw={"retries": retry},
        )
    ...
    def _to_response(self, data: _BatchItem, delay: timedelta):
        return rx.of(data).pipe(
            ...
            ops.catch(
                handler=lambda exception, source: rx.just(
                    _BatchResponse(exception=exception, data=data)
                )
            ),
        )
        ...
        if response.exception:
            ...
            if self._error_callback:
                try:
                    ...
                    """
                    logger.error(
                        "The configured error callback threw an exception: %s", e
                    )
            ...
            if self._success_callback:
                try:
                    self._success_callback(
                        response.data.to_key_tuple(), response.data.data
                    )
                except Exception as e:
                    logger.error(
                        "The configured success callback threw an exception: %s", e
                    )
