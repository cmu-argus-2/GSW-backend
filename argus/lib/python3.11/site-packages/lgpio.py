    ...
    def run(self):
        ...
        while self.go:
            ...
            while self.go and (len(buf) - offset) >= MSG_SIZ:
                ...
                offset += MSG_SIZ
                tick, chip, gpio, level, flags, pad = struct.unpack("QBBBBI", msgbuf)
...
def get_module_version():
    ...
    return "lgpio.py_{}.{}.{}.{}".format(
        (LGPIO_PY_VERSION >> 24) & 0xFF,
        (LGPIO_PY_VERSION >> 16) & 0xFF,
        (LGPIO_PY_VERSION >> 8) & 0xFF,
        LGPIO_PY_VERSION & 0xFF,
    )
...
def group_claim_output(handle, gpio, levels=[0], lFlags=0):
    ...
    if len(gpio):
        ...
        return _u2i(_lgpio._group_claim_output(handle & 0xFFFF, lFlags, GPIO, LEVELS))
    else:
        return 0
...
def group_write(handle, gpio, group_bits, group_mask=GROUP_ALL):
    ...
    """
    return _u2i(_lgpio._group_write(handle & 0xFFFF, gpio, group_bits, group_mask))


def tx_pulse(handle, gpio, pulse_on, pulse_off, pulse_offset=0, pulse_cycles=0):
    """
...
def tx_pwm(handle, gpio, pwm_frequency, pwm_duty_cycle, pulse_offset=0, pulse_cycles=0):
    """
...
def tx_servo(
    handle, gpio, pulse_width, servo_frequency=50, pulse_offset=0, pulse_cycles=0
):
    """
...
def tx_wave(handle, gpio, pulses):
    ...
    if len(pulses):
        PULSES = bytearray()
        for p in pulses:
            PULSES.extend(struct.pack("QQQ", p.group_bits, p.group_mask, p.pulse_delay))
...
def gpio_set_debounce_micros(handle, gpio, debounce_micros):
    ...
    """
    return _u2i(
        _lgpio._gpio_set_debounce_micros(handle & 0xFFFF, gpio, debounce_micros)
    )
...
def gpio_set_watchdog_micros(handle, gpio, watchdog_micros):
    ...
    """
    return _u2i(
        _lgpio._gpio_set_watchdog_micros(handle & 0xFFFF, gpio, watchdog_micros)
    )


def gpio_claim_alert(handle, gpio, eFlags, lFlags=0, notify_handle=None):
    """
    ...
    return _u2i(
        _lgpio._gpio_claim_alert(handle & 0xFFFF, lFlags, eFlags, gpio, notify_handle)
    )
...
def i2c_write_i2c_block_data(handle, reg, data):
    ...
    ...
    """
    return _u2i(_lgpio._i2c_write_i2c_block_data(handle, reg, _tobuf(data)))
...
def _tx_pwm(handle, gpio, pwmFrequency, pwmDutyCycle, pwmOffset, pwmCycles):
    return _lgpio._tx_pwm(
        handle, gpio, pwmFrequency, pwmDutyCycle, pwmOffset, pwmCycles
    )


def _tx_servo(handle, gpio, pulseWidth, servoFrequency, servoOffset, servoCycles):
    return _lgpio._tx_servo(
        handle, gpio, pulseWidth, servoFrequency, servoOffset, servoCycles
    )
