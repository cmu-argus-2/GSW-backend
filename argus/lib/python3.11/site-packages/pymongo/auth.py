...
from pymongo.auth_oidc import (_authenticate_oidc, _get_authenticator,
                               _OIDCAzureCallback, _OIDCGCPCallback,
                               _OIDCProperties, _OIDCTestCallback)
...
def _build_credentials_tuple(
    ...
    if mech == "GSSAPI":
        ...
        if source is not None and source != "$external":
            raise ValueError(
                "authentication source must be $external or None for MONGODB-X509"
            )
        ...
        msg = "authentication with MONGODB-OIDC requires providing either a callback or a environment"
        if passwd is not None:
        ...
        if callback or human_callback:
            ...
            if environ == "test":
                ...
                raise ConfigurationError(
                    f"unrecognized ENVIRONMENT for MONGODB-OIDC: {environ}"
                )
        else:
            raise ConfigurationError(msg)
        ...
        try:
            ...
            if (
                kerberos.authGSSClientWrap(
                    ctx, kerberos.authGSSClientResponse(ctx), username
                )
                != 1
            ):
