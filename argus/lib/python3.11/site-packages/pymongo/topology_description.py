...
from typing import (Any, Callable, List, Mapping, MutableMapping, NamedTuple,
                    Optional, cast)
...
class TopologyDescription:
    ...
    def reset(self) -> TopologyDescription:
        ...
        sds = {
            address: ServerDescription(address) for address in self._server_descriptions
        }
    ...
    def has_known_servers(self) -> bool:
        ...
        return any(
            s for s in self._server_descriptions.values() if s.is_server_type_known
        )
    ...
    def has_readable_server(
        self, read_preference: _ServerMode = ReadPreference.PRIMARY
    ) -> bool:
...
def _update_rs_from_primary(
    ...
    if (
        server_description.max_wire_version is None
        or server_description.max_wire_version < 17
    ):
        new_election_tuple: tuple = (
        ...
        if None not in new_election_tuple:
            if (
                ...
                and new_election_tuple < max_election_tuple
            ):
        ...
        new_election_safe = tuple(
            MinKey() if i is None else i for i in new_election_tuple
        )
        max_election_safe = tuple(
            MinKey() if i is None else i for i in max_election_tuple
        )
