...
from typing import (TYPE_CHECKING, Any, Callable, ContextManager, Generic,
                    Iterable, Iterator, Mapping, MutableMapping, NoReturn,
...
from pymongo.operations import (DeleteMany, DeleteOne, IndexModel, InsertOne,
                                ReplaceOne, SearchIndexModel, UpdateMany,
                                UpdateOne, _IndexKeyHint, _IndexList, _Op)
...
from pymongo.typings import (_CollationIn, _DocumentType, _DocumentTypeArg,
                             _Pipeline)
...
class Collection(common.BaseObject, Generic[_DocumentType]):
        ...
        **kwargs: Any,
        ...
        if create or kwargs or collation:
            if encrypted_fields:
                ...
                self.__create(
                    _esc_coll_name(encrypted_fields, name),
                    opts,
                    None,
                    session,
                    qev2_required=True,
                ...
                self.__create(
                    _ecoc_coll_name(encrypted_fields, name), opts, None, session
                )
                self.__create(
                    name, kwargs, collation, session, encrypted_fields=encrypted_fields
                )
    ...
    def _retryable_non_cursor_read(
        self,
        func: Callable[
            [Optional[ClientSession], Server, Connection, Optional[_ServerMode]], T
        ],
        session: Optional[ClientSession],
        operation: str,
    ) -> T:
        ...
        with client._tmp_session(session) as s:
            return client._retryable_read(
                func, self._read_preference_for(s), s, operation
            )
    ...
    def list_indexes(
        ...
        read_pref = (
            session and session._txn_read_preference()
        ...
        def _cmd(
            ...
            try:
                cursor = self._command(
                    conn, cmd, read_preference, codec_options, session=session
                )["cursor"]
