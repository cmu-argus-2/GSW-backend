...
from pymongo.errors import (NetworkTimeout, NotPrimaryError, OperationFailure,
                            _OperationCancelled)
...
class MonitorBase:
    def __init__(
        self, topology: Topology, name: str, interval: int, min_interval: float
    ):
...
class Monitor(MonitorBase):
    def __init__(
        ...
        self._publish = (
            self._listeners is not None and self._listeners.enabled_for_server_heartbeat
        )
    ...
    def _run(self) -> None:
        try:
            ...
            self._topology.on_change(
                ...
                interrupt_connections=isinstance(
                    self._server_description.error, NetworkTimeout
                ),
            )
    ...
    def _check_server(self) -> ServerDescription:
        ...
        try:
            ...
            if self._publish:
                awaited = bool(
                    self._stream and sd.is_server_type_known and sd.topology_version
                )
    ...
    def _check_once(self) -> ServerDescription:
        ...
        with self._pool.checkout() as conn:
            ...
            sd = ServerDescription(
                address, response, avg_rtt, min_round_trip_time=min_rtt
            )
            if self._publish:
                assert self._listeners is not None
