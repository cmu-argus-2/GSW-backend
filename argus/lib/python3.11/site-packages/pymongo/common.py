...
from typing import (TYPE_CHECKING, Any, Callable, Iterator, Mapping,
                    MutableMapping, NoReturn, Optional, Sequence, Type, Union,
                    overload)
...
def validate_non_negative_int_or_basestring(option: Any, value: Any) -> Union[int, str]:
    ...
    raise TypeError(
        f"Wrong type for {option}, value must be an non negative integer or a string"
    )
...
def validate_auth_mechanism_properties(
    ...
    if not isinstance(value, str):
        if not isinstance(value, dict):
            raise ValueError(
                "Auth mechanism properties must be given as a string or a dictionary"
            )
        for key, value in value.items():  # noqa: B020
            if isinstance(value, str):
                ...
                raise ValueError(
                    f"Invalid type for auth mechanism property {key}, {type(value)}"
                )
        return props
...
def validate_auth_option(option: str, value: Any) -> tuple[str, Any]:
    ...
    if lower not in _AUTH_OPTIONS:
        raise ConfigurationError(
            f"Unknown option: {option}. Must be in {_AUTH_OPTIONS}"
        )
    return option, value
...
def _get_validator(
    key: str,
    validators: dict[str, Callable[[Any, Any], Any]],
    normed_key: Optional[str] = None,
) -> Callable:
    ...
    for opt, value in options.items():
        ...
        try:
            validator = _get_validator(
                opt, URI_OPTIONS_VALIDATOR_MAP, normed_key=normed_key
            )
...
WRITE_CONCERN_OPTIONS = frozenset(
    ["w", "wtimeout", "wtimeoutms", "fsync", "j", "journal"]
)
