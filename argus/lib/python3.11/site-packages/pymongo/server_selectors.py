...
from typing import (TYPE_CHECKING, Any, Mapping, Optional, Sequence, TypeVar,
                    cast)
...
class Selection:
    ...
    def from_topology_description(
        cls, topology_description: TopologyDescription
    ) -> Selection:
    ...
    def with_server_descriptions(
        self, server_descriptions: list[ServerDescription]
    ) -> Selection:
        return Selection(
    ...
    def secondary_with_max_last_write_date(self) -> Optional[ServerDescription]:
        ...
        if secondaries.server_descriptions:
            return max(
                secondaries.server_descriptions,
                key=lambda sd: cast(float, sd.last_write_date),
            )
        return None
...
def secondary_with_tags_server_selector(
    tag_sets: TagSets, selection: Selection
) -> Selection:
