...
class LogMessage:
    ...
    def _is_sensitive(self, doc_name: str) -> bool:
        ...
        is_sensitive_command = (
            "commandName" in self._kwargs
            and self._kwargs["commandName"] in _SENSITIVE_COMMANDS
        )

        is_sensitive_hello = (
            self._kwargs["commandName"] in _HELLO_COMMANDS
    ...
    def _redact(self) -> None:
        ...
        if "durationMS" in self._kwargs and hasattr(
            self._kwargs["durationMS"], "total_seconds"
        ):
            self._kwargs["durationMS"] = (
                self._kwargs["durationMS"].total_seconds() * 1000
            )
        if "serviceId" in self._kwargs:
            self._kwargs["serviceId"] = str(self._kwargs["serviceId"])
        document_length = int(
            os.getenv("MONGOB_LOG_MAX_DOCUMENT_LENGTH", _DEFAULT_DOCUMENT_LENGTH)
        )
        ...
        for doc_name in _DOCUMENT_NAMES:
            ...
            if doc:
                if doc_name == "failure" and is_server_side_error:
                    doc = {
                        k: v for k, v in doc.items() if k in _REDACTED_FAILURE_FIELDS
                    }
                ...
                if len(doc) > document_length:
                    doc = (
                        doc.encode()[:document_length].decode(
