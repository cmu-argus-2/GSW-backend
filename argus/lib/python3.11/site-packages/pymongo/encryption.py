...
from typing import (TYPE_CHECKING, Any, Dict, Generic, Iterator, Mapping,
                    MutableMapping, Optional, Sequence, Union, cast)

try:
    from pymongocrypt.auto_encrypter import \
        AutoEncrypter  # type:ignore[import]
    ...
    from pymongocrypt.mongocrypt import \
        MongoCryptOptions  # type:ignore[import]
    from pymongocrypt.state_machine import \
...
from pymongo.errors import (ConfigurationError, EncryptedCollectionError,
                            EncryptionError, InvalidOperation, PyMongoError,
...
_HTTPS_PORT = 443
_KMS_CONNECT_TIMEOUT = (
    CONNECT_TIMEOUT  # CDRIVER-3262 redefined this value to CONNECT_TIMEOUT
)
    ...
    def __init__(self, client: MongoClient[_DocumentTypeArg], opts: AutoEncryptionOpts):
        ...
        if opts._encrypted_fields_map is None:
            ...
            encrypted_fields_map = _dict_to_bson(
                opts._encrypted_fields_map, False, _DATA_KEY_OPTS
            )
...
class ClientEncryption(Generic[_DocumentType]):
    ...
    def rewrap_many_data_key(
        ...
        if master_key is not None and provider is None:
            raise ConfigurationError(
                "A provider must be given if a master_key is given"
            )
        self._check_closed()
        with _wrap_encryption_errors():
            raw_result = self._encryption.rewrap_many_data_key(
