...
from typing import (TYPE_CHECKING, Any, Generic, Mapping, MutableMapping,
                    NoReturn, Optional, Sequence, TypeVar, Union, cast,
                    overload)
...
class Database(common.BaseObject, Generic[_DocumentType]):
    ...
    def command(
        ...
        **kwargs: Any,
        ...
        with self.__client._conn_for_reads(
        ...
        **kwargs: Any,
        ...
        with self.__client._tmp_session(session, close=False) as tmp_session:
            cursor = self._command(
                conn, cmd, read_preference=read_preference, session=tmp_session
            )["cursor"]
            cmd_cursor = CommandCursor(
                coll,
                cursor,
    ...
    def drop_collection(
        ...
        if encrypted_fields:
            ...
            self._drop_helper(
                _ecoc_coll_name(encrypted_fields, name),
                session=session,
                comment=comment,
            )
    ...
    def validate_collection(
        ...
        if not isinstance(name, str):
            raise TypeError(
                "name_or_collection must be an instance of str or Collection"
            )
