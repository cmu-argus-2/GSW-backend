...
from typing import (TYPE_CHECKING, Any, Generic, Iterable, List, Mapping,
                    NoReturn, Optional, Sequence, Tuple, Union, cast, overload)
...
from pymongo.message import (_CursorAddress, _GetMore, _OpMsg, _OpReply,
                             _Query, _RawBatchGetMore, _RawBatchQuery)
...
class Cursor(Generic[_DocumentType]):
    ...
    def __die(self, synchronous: bool = False) -> None:
        ...
        if self.__id and not already_killed:
            ...
            address = _CursorAddress(
                self.__address, f"{self.__dbname}.{self.__collname}"
            )
        else:
            # Skip killCursors.
            cursor_id = 0
    ...
    def __getitem__(
        ...
        if isinstance(index, slice):
            ...
            if index.stop is not None:
                ...
                if limit < 0:
                    raise IndexError(
                        "stop index must be greater than start index for slice %r"
                        % index
                    )
                if limit == 0:
                    self.__empty = True
    ...
    def __send_message(self, operation: Union[_Query, _GetMore]) -> None:
        ...
        if isinstance(response, PinnedResponse):
            if not self.__sock_mgr:
                self.__sock_mgr = _ConnectionManager(
                    response.conn, response.more_to_come
                )
    ...
    def _unpack_response(
        ...
        return response.unpack_response(
            cursor_id, codec_options, user_fields, legacy_response
        )
    ...
    def _deepcopy(
        ...
        memo: Optional[dict[int, Union[list, dict]]] = None,
    ) -> Union[list, dict]:
