...
from pymongo.errors import (ConnectionFailure, InvalidOperation,
                            NetworkTimeout, NotPrimaryError, OperationFailure,
                            PyMongoError, ServerSelectionTimeoutError,
                            WriteError)
from pymongo.hello import Hello
from pymongo.lock import _create_lock
from pymongo.logger import (_SERVER_SELECTION_LOGGER, _debug_log,
...
class Topology:
    ...
    def __init__(self, topology_settings: TopologySettings):
        ...
        self._publish_server = (
            self._listeners is not None and self._listeners.enabled_for_server
        )
        self._publish_tp = (
            self._listeners is not None and self._listeners.enabled_for_topology
        )
    ...
    def _process_change(
        ...
        if server_description.is_readable or (
            ...
            and new_td.topology_type == TOPOLOGY_TYPE.Single
        ):
        ...
        suppress_event = (
            self._publish_server or self._publish_tp
    ...
    def _receive_cluster_time_no_lock(
        self, cluster_time: Optional[Mapping[str, Any]]
    ) -> None:
    ...
    def _ensure_opened(self) -> None:
        ...
        if not self._opened:
            ...
            if self._srv_monitor and (
                self.description.topology_type in SRV_POLLING_TOPOLOGIES
            ):
    ...
    def _error_message(self, selector: Callable[[Selection], Selection]) -> str:
        ...
        if self._description.known_servers:
            ...
            if same:
                ...
                if is_replica_set and not set(addresses).intersection(
    ...
    def eq_props(
        self,
    ) -> tuple[tuple[_Address, ...], Optional[str], Optional[str], str]:
        """The properties to use for MongoClient/Topology equality checks."""
        ts = self._settings
        return (
            tuple(sorted(ts.seeds)),
            ts.replica_set_name,
            ts.fqdn,
            ts.srv_service_name,
        )
...
def _is_stale_server_description(
    current_sd: ServerDescription, new_sd: ServerDescription
) -> bool:
