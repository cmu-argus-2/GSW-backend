...
from typing import (TYPE_CHECKING, Any, Callable, ContextManager, FrozenSet,
                    Generic, Iterator, Mapping, MutableMapping, NoReturn,
                    Optional, Sequence, Type, TypeVar, Union, cast)

from bson.codec_options import (DEFAULT_CODEC_OPTIONS, CodecOptions,
                                TypeRegistry)
from bson.timestamp import Timestamp
from pymongo import (_csot, client_session, common, database, helpers, message,
...
from pymongo.typings import (ClusterTime, _Address, _CollationIn,
                             _DocumentType, _DocumentTypeArg, _Pipeline)
...
_ReadCall = Callable[
    [Optional["ClientSession"], "Server", "Connection", _ServerMode], T
]
...
class MongoClient(common.BaseObject, Generic[_DocumentType]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        keyword_opts = common._CaseInsensitiveDictionary(
            dict(
                common.validate(keyword_opts.cased_key(k), v)
        ...
        try:
            ...
            with self._conn_for_reads(
                ...
                for i in range(0, len(session_ids), common._MAX_END_SESSIONS):
                    spec = {
                        "endSessions": session_ids[i : i + common._MAX_END_SESSIONS]
                    }
    ...
    def _select_server(
        ...
        try:
            ...
            if address:
                ...
                if not server:
                    raise AutoReconnect(
                        "server %s:%s no longer available" % address
                    )  # noqa: UP031
            else:
    ...
    def _retryable_write(
        ...
        with self._tmp_session(session) as s:
            return self._retry_with_session(
                retryable, func, s, bulk, operation, operation_id
            )
    ...
    def list_database_names(
        ...
        return [
            doc["name"]
            for doc in self.list_databases(session, nameOnly=True, comment=comment)
        ]
...
def _add_retryable_write_error(
    exc: PyMongoError, max_wire_version: int, is_mongos: bool
) -> None:
...
class _MongoClientErrorHandler:
    ...
    def __init__(
        self, client: MongoClient, server: Server, session: Optional[ClientSession]
    ):
        self.client = client
    ...
    def contribute_socket(
        self, conn: Connection, completed_handshake: bool = True
    ) -> None:
...
class _ClientConnectionRetryable(Generic[T]):
            ...
            try:
                ...
                if not self._is_read:
                    if not self._retryable:
                        raise
                    retryable_write_error_exc = exc.has_error_label(
                        "RetryableWriteError"
                    )
                    ...
                    if (
                        ...
                        or self._is_not_eligible_for_retry()
                    ):
                        if (
                            exc.has_error_label("NoWritesPerformed")
                ...
                if (
                    self._client.topology_description.topology_type
                    == TOPOLOGY_TYPE.Sharded
                ):
    ...
    def _is_not_eligible_for_retry(self) -> bool:
        ...
        return not self._retryable or (
            self._is_retrying() and not self._multiple_retries
        )
    ...
    def _read(self) -> T:
        ...
        with self._client._conn_from_server(
            self._read_pref, self._server, self._session
        ) as (
            conn,
            read_pref,
        ):
