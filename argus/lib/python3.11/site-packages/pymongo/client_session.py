...
from typing import (TYPE_CHECKING, Any, Callable, ContextManager, Mapping,
                    MutableMapping, NoReturn, Optional, Type, TypeVar)
        ...
        if snapshot:
            if causal_consistency:
                raise ConfigurationError(
                    "snapshot reads do not support causal_consistency=True"
                )
...
class ClientSession:
    ...
    def session_id(self) -> Mapping[str, Any]:
        ...
        self._check_ended()
        self._materialize(
            self._client.topology_description.logical_session_timeout_minutes
        )
    ...
    def _finish_transaction(
        self, conn: Connection, command_name: str
    ) -> dict[str, Any]:
    ...
    def advance_operation_time(self, operation_time: Timestamp) -> None:
        ...
        if not isinstance(operation_time, Timestamp):
            raise TypeError(
                "operation_time must be an instance of bson.timestamp.Timestamp"
            )
    ...
    def __copy__(self) -> NoReturn:
        raise TypeError(
            "A ClientSession cannot be copied, create a new session instead"
        )
