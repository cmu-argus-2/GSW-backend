...
from typing import (TYPE_CHECKING, Any, Callable, Iterable, Mapping,
                    MutableMapping, NoReturn, Optional, Union)

import bson
from bson import (CodecOptions, _decode_selective, _dict_to_bson,
                  _make_c_string, encode)
from bson.int64 import Int64
from bson.raw_bson import (_RAW_ARRAY_BSON_OPTIONS, DEFAULT_RAW_BSON_OPTIONS,
                           RawBSONDocument, _inflate_bson)
...
if TYPE_CHECKING:
    ...
    from pymongo.compression_support import (SnappyContext, ZlibContext,
                                             ZstdContext)
    ...
    def use_command(self, conn: Connection) -> bool:
        ...
        if not self.exhaust:
            ...
            raise ConfigurationError(
                ...
                % (self.read_concern.level, conn.max_wire_version)
            )
...
class _BulkWriteContext:
    ...
    def _succeed(
        self, request_id: int, reply: _DocumentOut, duration: timedelta
    ) -> None:
    ...
    def _fail(
        self, request_id: int, failure: _DocumentOut, duration: timedelta
    ) -> None:
...
def _batched_write_command_impl(
    ...
    for doc in docs:
        ...
        if doc_too_large:
            ...
            _raise_document_too_large(write_op, len(value), max_bson_size)
        enough_data = (
            idx >= 1 and (buf.tell() + len(key) + len(value)) >= max_split_size
        )
...
class _OpReply:
    ...
    def __init__(
        self, flags: int, cursor_id: int, number_returned: int, documents: bytes
    ):
        self.flags = flags
        ...
        if self.flags & 1:
            ...
            if error_object["$err"].startswith(HelloCompat.LEGACY_ERROR):
                ...
                raise ExecutionTimeout(
                    error_object.get("$err", default_msg),
    ...
    def unpack(cls, msg: bytes) -> _OpMsg:
        ...
        if flags != 0:
            if flags & cls.CHECKSUM_PRESENT:
                raise ProtocolError(
                    f"Unsupported OP_MSG flag checksumPresent: 0x{flags:x}"
                )
        ...
        if first_payload_type != 0:
            raise ProtocolError(
                f"Unsupported OP_MSG payload type: 0x{first_payload_type:x}"
            )
