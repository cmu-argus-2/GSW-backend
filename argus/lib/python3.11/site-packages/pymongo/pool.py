...
from typing import (TYPE_CHECKING, Any, Iterator, Mapping, MutableMapping,
                    NoReturn, Optional, Sequence, Union)
...
from pymongo.common import (MAX_BSON_SIZE, MAX_CONNECTING, MAX_IDLE_TIME_SEC,
                            ...
                            WAIT_QUEUE_TIMEOUT)
from pymongo.errors import (AutoReconnect,  # type:ignore[attr-defined]
                            ...
                            WaitQueueTimeoutError, _CertificateError)
from pymongo.hello import Hello, HelloCompat
from pymongo.helpers import _handle_reauth
from pymongo.lock import _create_lock
from pymongo.logger import (_CONNECTION_LOGGER, _ConnectionStatusMessage,
                            _debug_log, _verbose_connection_error_reason)
from pymongo.monitoring import (ConnectionCheckOutFailedReason,
                                ConnectionClosedReason, _EventListeners)
...
class Connection:
    ...
    def __init__(
        self,
        conn: Union[socket.socket, _sslConn],
        pool: Pool,
        ...
        self.compression_context: Union[
            SnappyContext, ZlibContext, ZstdContext, None
        ] = None
    ...
    def _hello(
        ...
        self.logical_session_timeout_minutes: Optional[
            int
        ] = hello.logical_session_timeout_minutes
    ...
    def command(
        ...
        if not (
            write_concern is None or write_concern.acknowledged or collation is None
        ):
            raise ConfigurationError(
...
class Pool:
    ...
    def remove_stale_sockets(self, reference_generation: int) -> None:
        ...
        if self.opts.max_idle_time_seconds is not None:
            with self.lock:
                while (
                    self.conns
                    and self.conns[-1].idle_time_seconds()
    ...
    def _get_conn(
        self,
        checkout_started_time: float,
        handler: Optional[_MongoClientErrorHandler] = None,
    ) -> Connection:
                ...
                with self._max_connecting_cond:
                    ...
                    while not (self.conns or self._pending < self._max_connecting):
                        ...
                        self._raise_if_not_ready(
                            checkout_started_time, emit_event=False
                        )
