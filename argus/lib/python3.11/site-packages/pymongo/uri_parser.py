...
def _handle_option_deprecations(
    options: _CaseInsensitiveDictionary,
) -> _CaseInsensitiveDictionary:
        ...
        if optname in URI_OPTIONS_DEPRECATION_MAP:
            ...
            if mode == "renamed":
                ...
                if newoptname in options:
                    ...
                    warnings.warn(
                        warn_msg
                        % (options.cased_key(optname), options.cased_key(newoptname)),
                        DeprecationWarning,
                        stacklevel=2,
                    )
                    options.pop(optname)
...
def parse_uri(
    ...
    if uri.startswith(SCHEME):
        ...
        raise InvalidURI(
            f"Invalid URI scheme: URI must begin with '{SCHEME}' or '{SRV_SCHEME}'"
        )
    ...
    if is_srv:
        if options.get("directConnection"):
            raise ConfigurationError(
                f"Cannot specify directConnection=true with {SRV_SCHEME} URIs"
            )
        nodes = split_hosts(hosts, default_port=None)
        if len(nodes) != 1:
            raise InvalidURI(
                f"{SRV_SCHEME} URIs must include one, and only one, hostname"
            )
        fqdn, port = nodes[0]
        if port is not None:
        ...
        dns_resolver = _SrvResolver(
            fqdn, connect_timeout, srv_service_name, srv_max_hosts
        )
