...
from typing import Any, AsyncGenerator, Generator, Iterable, List, Union

from influxdb_client import (ArrayExpression, BooleanLiteral, Configuration,
                             DateTimeLiteral, DeletePredicateRequest,
                             DeleteService, Dialect, Duration, DurationLiteral,
                             Expression, File, FloatLiteral, Identifier,
                             ImportDeclaration, IntegerLiteral,
                             MemberAssignment, MemberExpression,
                             OptionStatement, Query, QueryService,
                             StringLiteral, UnaryExpression,
                             VariableAssignment, WriteService)
from influxdb_client.client.flux_csv_parser import (FluxCsvParser,
                                                    FluxResponseMetadataMode,
                                                    FluxSerializationMode)
from influxdb_client.client.flux_table import (CSVIterator, FluxRecord,
                                               TableList)
from influxdb_client.client.util.date_utils import get_date_helper
from influxdb_client.client.util.helpers import get_org_query_param
from influxdb_client.client.warnings import MissingPivotFunction
from influxdb_client.client.write.dataframe_serializer import \
    DataframeSerializer
...
class _BaseClient(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.conf.connection_pool_maxsize = kwargs.get(
            "connection_pool_maxsize", self.conf.connection_pool_maxsize
        )
    ...
    def _from_config_file(
        ...
        return cls(
            ...
            auth_basic=_to_bool(auth_basic),
            profilers=profilers,
            proxy=proxy,
            **kwargs,
        )
...
class _BaseQueryApi(object):
    ...
    def _to_tables(
        ...
        """
        _parser = self._to_tables_parser(
            response, query_options, response_metadata_mode
        )
        list(_parser.generator())
        return _parser.table_list()

    async def _to_tables_async(
        self,
        response,
        query_options=None,
        response_metadata_mode: FluxResponseMetadataMode = FluxResponseMetadataMode.full,
        ...
        """
        async with self._to_tables_parser(
            response, query_options, response_metadata_mode
        ) as parser:
    ...
    def _to_flux_record_stream(
        ...
        :param response: HTTP response from an HTTP client. Expected type: `urllib3.response.HTTPResponse`.
        """
        _parser = self._to_flux_record_stream_parser(
            query_options, response, response_metadata_mode
        )
        return _parser.generator()

    async def _to_flux_record_stream_async(
        self,
        response,
        query_options=None,
        response_metadata_mode: FluxResponseMetadataMode = FluxResponseMetadataMode.full,
    ...
    def _to_data_frame_stream(
        ...
        :param response: HTTP response from an HTTP client. Expected type: `urllib3.response.HTTPResponse`.
        """
        _parser = self._to_data_frame_stream_parser(
            data_frame_index,
            query_options,
            response,
    ...
    async def _to_data_frame_stream_async(
        self,
        data_frame_index,
        response,
        query_options=None,
    ...
    def _to_flux_record_stream_parser(
        self, query_options, response, response_metadata_mode
    ):
        return FluxCsvParser(
            response=response,
    ...
    def _to_data_frame_stream_parser(
        self,
        data_frame_index,
        query_options,
        response,
    ...
    def _create_query(
        self, query, dialect=default_dialect, params: dict = None, **kwargs
    ):
        ...
        for key, value in params.items():
            ...
            statements.append(
                OptionStatement(
                    "OptionStatement",
                    VariableAssignment(
                        "VariableAssignment", Identifier("Identifier", key), expression
                    ),
                )
            )
        return statements
    ...
    def _parm_to_extern_ast(value) -> Union[Expression, None]:
        ...
        if isinstance(value, bool):
            ...
            return DateTimeLiteral(
                "DateTimeLiteral", value.strftime("%Y-%m-%dT%H:%M:%S.") + fraction + "Z"
            )
            ...
            if _micro_delta < 0:
                return UnaryExpression(
                    "UnaryExpression",
                    argument=DurationLiteral(
                        "DurationLiteral",
                        [Duration(magnitude=-_micro_delta, unit="us")],
                    ),
                    operator="-",
                )
            else:
                return DurationLiteral(
                    "DurationLiteral", [Duration(magnitude=_micro_delta, unit="us")]
                )
            ...
            return ArrayExpression(
                "ArrayExpression",
                elements=list(
                    map(lambda it: _BaseQueryApi._parm_to_extern_ast(it), value)
                ),
            )
        else:
            return value
    ...
    def _build_flux_ast(params: dict = None, profilers: List[str] = None):
        ...
        if profilers is not None and len(profilers) > 0:
            imports.append(
                ImportDeclaration(
                    "ImportDeclaration", path=StringLiteral("StringLiteral", "profiler")
                )
            )
            ...
            prof = OptionStatement(
                ...
                assignment=MemberAssignment(
                    "MemberAssignment",
                    member=member,
                    init=ArrayExpression("ArrayExpression", elements=elements),
                ),
            )
...
class _BaseWriteApi(object):
    ...
    def _serialize(self, record, write_precision, payload, **kwargs):
        ...
        if isinstance(record, bytes):
            ...
            self._serialize(
                record.encode(_UTF_8_encoding), write_precision, payload, **kwargs
            )

        elif isinstance(record, Point):
            precision_from_point = kwargs.get("precision_from_point", True)
            precision = (
                record.write_precision if precision_from_point else write_precision
            )
            self._serialize(
                record.to_line_protocol(precision=precision),
                precision,
                payload,
                **kwargs,
            )
            ...
            self._serialize(
                Point.from_dict(record, write_precision=write_precision, **kwargs),
                write_precision,
                payload,
                **kwargs,
            )
            ...
            serializer = DataframeSerializer(
                record, self._point_settings, write_precision, **kwargs
            )
            ...
            self._serialize(
                dataclasses.asdict(record), write_precision, payload, **kwargs
            )
        elif isinstance(record, Iterable):
            for item in record:
...
class _BaseDeleteApi(object):
    ...
    def _prepare_predicate_request(self, start, stop, predicate):
        ...
        predicate_request = DeletePredicateRequest(
            start=start, stop=stop, predicate=predicate
        )
