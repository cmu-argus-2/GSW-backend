...
from influxdb_client.client.flux_table import (FluxColumn, FluxRecord,
                                               FluxTable, TableList)
...
class FluxCsvParser(object):
    ...
    def __init__(
        ...
        self._profiler_callback = (
            query_options.profiler_callback if query_options is not None else None
        )
        self._async_mode = (
            True if "ClientResponse" in type(response).__name__ else False
        )
    ...
    def __enter__(self):
        ...
        if hasattr(self._response, "closed") and self._response.closed:
            ...
            self._reader = csv_parser.reader(
                StringIO(self._response.data.decode(_UTF_8_encoding))
            )
        else:
            self._reader = csv_parser.reader(
                codecs.iterdecode(self._response, _UTF_8_encoding)
            )
        return self
    ...
    def _parse_flux_response(self):
        ...
        if (self._serialization_mode is FluxSerializationMode.dataFrame) & hasattr(
            self, "_data_frame"
        ):
    ...
    def _parse_flux_response_row(self, metadata, csv):
        if len(csv) < 1:
            ...
            if (token in ANNOTATIONS and not metadata.start_new_table) or (
                ...
                and not metadata.table
            ):
                # Return already parsed DataFrame
                if (
                    self._serialization_mode is FluxSerializationMode.dataFrame
            ...
            if ANNOTATION_DATATYPE == token:
                ...
                if metadata.start_new_table:
                    ...
                    if (
                        not metadata.table.columns
                        and self._response_metadata_mode
                        is FluxResponseMetadataMode.only_names
                    ):
                        self.add_data_types(
                            metadata.table, list(map(lambda column: "string", csv))
                        )
                    ...
                    if self._serialization_mode is FluxSerializationMode.dataFrame:
                        ...
                        self._data_frame = pd.DataFrame(
                            data=[], columns=labels, index=None
                        )
                        pass
                else:
                    ...
                    flux_record = self.parse_record(
                        metadata.table_index - 1, metadata.table, csv
                    )
                    ...
                    if self._is_profiler_record(flux_record):
                        ...
                        if self._serialization_mode is FluxSerializationMode.tables:
                            self.tables[metadata.table_index - 1].records.append(
                                flux_record
                            )
    ...
    def _to_value(self, str_val, column):
        ...
        if (
            "dateTime:RFC3339" == column.data_type
            or "dateTime:RFC3339Nano" == column.data_type
        ):
...
"""
    ...
    def _is_profiler_record(self, flux_record: FluxRecord) -> bool:
        ...
        for profiler in self._profilers:
            if (
                "_measurement" in flux_record.values
                and flux_record["_measurement"] == "profiler/" + profiler
            ):
                return True
    ...
    def _is_profiler_table(self, table: FluxTable) -> bool:
        ...
        return any(
            filter(
                lambda column: (
                    column.default_value == "_profiler" and column.label == "result"
                ),
                table.columns,
            )
        )
    ...
    def table_list(self) -> TableList:
        ...
        if not self._profilers:
            ...
            return TableList(
                filter(lambda table: not self._is_profiler_table(table), self.tables)
            )
