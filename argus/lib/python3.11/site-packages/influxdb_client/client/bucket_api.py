...
from influxdb_client import (Bucket, BucketsService, PatchBucketRequest,
                             PostBucketRequest)
...
class BucketsApi(object):
    ...
    def update_bucket(self, bucket: Bucket) -> Bucket:
        ...
        :return: Bucket
        """
        request = PatchBucketRequest(
            name=bucket.name,
            description=bucket.description,
            retention_rules=bucket.retention_rules,
        )

        return self._buckets_service.patch_buckets_id(
            bucket_id=bucket.id, patch_bucket_request=request
        )
    ...
    def find_buckets_iter(self, **kwargs):
        ...
        """
        return _Paginated(
            self._buckets_service.get_buckets, lambda response: response.buckets
        ).find_iter(**kwargs)
