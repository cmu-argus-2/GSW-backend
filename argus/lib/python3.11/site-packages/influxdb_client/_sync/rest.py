...
from influxdb_client.rest import ApiException, _BaseRESTClient
        ...
        **urlopen_kw,
        ...
        if _configured_timeout:
            if isinstance(
                ...
                isinstance(_configured_timeout, tuple) and len(_configured_timeout) == 2
            ):
                timeout = urllib3.Timeout(
                    connect=_configured_timeout[0] / 1_000,
                    read=_configured_timeout[1] / 1_000,
                )
        ...
        if self.configuration.debug:
            _BaseRESTClient.log_request(
                method,
                f"{url}{'' if query_params is None else '?' + urlencode(query_params)}",
            )
