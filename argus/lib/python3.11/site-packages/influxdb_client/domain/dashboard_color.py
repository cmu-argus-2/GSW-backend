...
class DashboardColor(object):
    ...
    def __init__(
        self, id=None, type=None, hex=None, name=None, value=None
    ):  # noqa: E501,D401,D403
    ...
    def hex(self, hex):
        ...
        if hex is None:
            raise ValueError(
                "Invalid value for `hex`, must not be `None`"
            )  # noqa: E501
        if hex is not None and len(hex) > 7:
            raise ValueError(
                "Invalid value for `hex`, length must be less than or equal to `7`"
            )  # noqa: E501
        if hex is not None and len(hex) < 7:
            raise ValueError(
                "Invalid value for `hex`, length must be greater than or equal to `7`"
            )  # noqa: E501
        self._hex = hex
