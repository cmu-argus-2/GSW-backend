...
from typing import (IO, TYPE_CHECKING, Any, BinaryIO, Callable, Generator,
                    Iterator, Mapping, MutableMapping, NoReturn, Optional,
...
from bson.codec_options import (DEFAULT_CODEC_OPTIONS, CodecOptions,
                                DatetimeConversion, _raw_document_class)
from bson.datetime_ms import (EPOCH_AWARE, EPOCH_NAIVE, DatetimeMS,
                              _datetime_to_millis, _millis_to_datetime)
...
def _raise_unknown_type(element_type: int, element_name: str) -> NoReturn:
    ...
    raise InvalidBSON(
        ...
        "you using the latest driver version?".format(
...
_ELEMENT_GETTER: dict[int, Callable[..., Tuple[Any, int]]] = {
    ...
    ord(BSONUND): lambda u, v, w, x, y, z: (
        None,
        w,
...
def _raw_to_dict(
    ...
    return cast(
        _T,
        _elements_to_dict(
            data, view, position, obj_end, opts, result, raw_array=raw_array
        ),
    )
...
def _encode_uuid(
    name: bytes, value: uuid.UUID, dummy: Any, opts: CodecOptions[Any]
) -> bytes:
    """Encode uuid.UUID."""
...
def _encode_datetime(
    name: bytes, value: datetime.datetime, dummy0: Any, dummy1: Any
) -> bytes:
...
def _encode_datetime_ms(
    name: bytes, value: DatetimeMS, dummy0: Any, dummy1: Any
) -> bytes:
    """Encode datetime.datetime."""
    millis = int(value)
    return b"\x09" + name + _PACK_LONG(millis)
...
def _decode_selective(
    ...
    for key, value in rawdoc.items():
        if key in fields:
            if fields[key] == 1:
                doc[key] = _bson_to_dict(rawdoc.raw, codec_options)[
                    key
                ]  # type:ignore[index]
            else:
