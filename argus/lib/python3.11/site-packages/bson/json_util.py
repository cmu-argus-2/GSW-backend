...
from bson.datetime_ms import (EPOCH_AWARE, DatetimeMS, _datetime_to_millis,
                              _max_datetime_ms, _millis_to_datetime)
...
class JSONOptions(_BASE_CLASS):
    ...
    def __new__(
        ...
        **kwargs: Any,
        ...
        self = cast(
            JSONOptions, super().__new__(cls, *args, **kwargs)
...
def _parse_canonical_binary(
    ...
    if len(binary) != 2:
        raise TypeError(
            f'$binary must include only "base64" and "subType" components: {doc}'
        )
...
def _parse_canonical_dbref(doc: Any, dummy0: Any) -> Any:
    ...
    if (
        ...
        return DBRef(
            doc.pop("$ref"), doc.pop("$id"), database=doc.pop("$db", None), **doc
        )
    return doc
