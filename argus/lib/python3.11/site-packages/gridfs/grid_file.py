...
from pymongo.errors import (BulkWriteError, ConfigurationError, CursorNotFound,
...
def _grid_in_property(
    ...
    if docstring.startswith("DEPRECATED,"):
        warn_str = f"GridIn property '{field_name}' is deprecated and will be removed in PyMongo 5.0"
    ...
    def setter(self: Any, value: Any) -> Any:
        ...
        if self._closed:
            self._coll.files.update_one(
                {"_id": self._file["_id"]}, {"$set": {field_name: value}}
            )
...
def _grid_out_property(field_name: str, docstring: str) -> Any:
    ...
    warn_str = ""
    if docstring.startswith("DEPRECATED,"):
        warn_str = f"GridOut property '{field_name}' is deprecated and will be removed in PyMongo 5.0"
...
class GridIn:
        ...
        **kwargs: Any,
        ...
        if not root_collection.write_concern.acknowledged:
            raise ConfigurationError(
                "root_collection must use acknowledged write_concern"
            )
    ...
    _id: Any = _grid_in_property(
        "_id", "The ``'_id'`` value for this file.", read_only=True
    )
    ...
    length: int = _grid_in_property(
        "length", "Length (in bytes) of this file.", closed_only=True
    )
    chunk_size: int = _grid_in_property(
...
class GridOut(io.IOBase):
    ...
    def _ensure_file(self) -> None:
        if not self._file:
            _disallow_transactions(self._session)
            self._file = self.__files.find_one(
                {"_id": self.__file_id}, session=self._session
            )
            if not self._file:
                raise NoFile(
