...
try:
    from pyarrow._dataset import (CsvFileFormat, CsvFragmentScanOptions,
                                  Dataset, DatasetFactory,
                                  DirectoryPartitioning, FeatherFileFormat,
                                  FileFormat, FileFragment,
                                  FilenamePartitioning, FileSystemDataset,
                                  FileSystemDatasetFactory,
                                  FileSystemFactoryOptions, FileWriteOptions,
                                  Fragment, FragmentScanOptions,
                                  HivePartitioning, InMemoryDataset,
                                  IpcFileFormat, IpcFileWriteOptions,
                                  JsonFileFormat, JsonFragmentScanOptions,
                                  Partitioning, PartitioningFactory, Scanner,
                                  TaggedRecordBatch, UnionDataset,
                                  UnionDatasetFactory, WrittenFile,
                                  _filesystemdataset_write)
    from pyarrow._dataset import get_partition_keys  # noqa
    from pyarrow._dataset import \
        get_partition_keys as \
        _get_partition_keys  # keep for backwards compatibility
...
from pyarrow.compute import Expression, field, scalar  # noqa
...
try:
    from pyarrow._dataset_parquet import (ParquetDatasetFactory,  # noqa
                                          ...
                                          ParquetReadOptions, RowGroupInfo)
...
try:
    from pyarrow._dataset_parquet_encryption import (  # noqa
        ParquetDecryptionConfig, ParquetEncryptionConfig)
except ImportError:
    pass
...
def __getattr__(name):
    ...
    raise AttributeError("module 'pyarrow.dataset' has no attribute '{0}'".format(name))
    ...
    if flavor is None:
        ...
        if schema is not None:
            ...
            if isinstance(field_names, list):
                ...
                raise ValueError(
                    "Expected list of field names, got {}".format(type(field_names))
                )
        else:
            raise ValueError(
    ...
    if flavor == "filename":
        ...
        if field_names is not None:
            ...
            if isinstance(schema, pa.Schema):
                ...
                raise ValueError(
                    "Expected Schema for 'schema', got {}".format(type(schema))
                )
        else:
...
def _ensure_partitioning(scheme):
    ...
    if scheme is None:
        ...
        raise ValueError(
            "Expected Partitioning or PartitioningFactory, got {}".format(type(scheme))
        )
    return scheme
...
def _ensure_multiple_sources(paths, filesystem=None):
    ...
    IOError
        ...
        not a file.
    """
    from pyarrow.fs import (FileType, LocalFileSystem, SubTreeFileSystem,
                            _ensure_filesystem, _MockFileSystem)
...
def _filesystem_dataset(
    ...
    FileSystemDataset
    """
    from pyarrow.fs import FileInfo, LocalFileSystem, _ensure_filesystem
...
'delete_matching'
    ...
    if format != file_options.format:
        raise TypeError(
            "Supplied FileWriteOptions have format {}, "
            "which doesn't match supplied FileFormat {}".format(format, file_options)
        )
