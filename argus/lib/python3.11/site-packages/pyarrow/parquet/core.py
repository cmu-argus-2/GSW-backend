...
from pyarrow._parquet import Statistics  # noqa
from pyarrow._parquet import (ColumnChunkMetaData, ColumnSchema,
                              FileDecryptionProperties,
                              FileEncryptionProperties, FileMetaData,
                              ParquetLogicalType, ParquetReader, ParquetSchema,
                              RowGroupMetaData, SortingColumn)
from pyarrow.fs import (FileSystem, FileType, LocalFileSystem,
                        _ensure_filesystem, _resolve_filesystem_and_path)
from pyarrow.util import _deprecate_api, _is_path_like, _stringify_path, guid
...
{}
""".format(
        _parquet_writer_arg_docs, _parquet_writer_example_doc
    )
    ...
    def write_table(self, table, row_group_size=None):
        ...
        if not table.schema.equals(self.schema, check_metadata=False):
            msg = (
                ...
                "\ntable:\n{!s} vs. \nfile:\n{!s}".format(table.schema, self.schema)
            )
...
{2}
""".format(
        _read_docstring_common, _DNF_filter_doc, _parquet_dataset_example
    )
...
{}
""".format(
    _parquet_writer_arg_docs, _write_table_example
)
