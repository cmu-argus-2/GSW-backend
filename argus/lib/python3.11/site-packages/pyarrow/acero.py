...
try:
    from pyarrow._acero import (AggregateNodeOptions,  # noqa
                                AsofJoinNodeOptions, Declaration,
                                ExecNodeOptions, FilterNodeOptions,
                                HashJoinNodeOptions, OrderByNodeOptions,
                                ProjectNodeOptions, TableSourceNodeOptions)
...
def _perform_join_asof(
    ...
    if isinstance(left_operand, ds.Dataset):
        left_source = _dataset_to_decl(
            left_operand, use_threads=use_threads, require_sequenced_output=True
        )
    else:
    ...
    if isinstance(right_operand, ds.Dataset):
        right_source = _dataset_to_decl(
            right_operand, use_threads=use_threads, require_sequenced_output=True
        )
    else:
        right_source = Declaration(
            "table_source", TableSourceNodeOptions(right_operand)
