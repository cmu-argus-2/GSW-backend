...
def test_fragments_repr(tempdir, dataset):
    ...
    fragment = list(dataset.get_fragments())[0]
    assert repr(fragment) == "<pyarrow.dataset.ParquetFileFragment path={}>".format(
    ...
    fragment = list(dataset.get_fragments())[0]
    assert repr(fragment) == "<pyarrow.dataset.FileFragment type=ipc path={}>".format(
        dataset.filesystem.normalize_path(str(path))
    )
...
def s3_example_simple(s3_server):
    ...
    uri = (
        ...
        "&allow_bucket_creation=True".format(access_key, secret_key, host, port)
    )
...
def test_open_dataset_from_s3_with_filesystem_uri(s3_server):
    ...
    uri = (
        "s3://{}:{}@{}/{}?scheme=http&endpoint_override={}:{}"
