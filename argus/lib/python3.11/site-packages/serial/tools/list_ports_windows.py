...
import re
from ctypes.wintypes import BOOL, BYTE, DWORD, HKEY, HWND, LONG, ULONG, WORD
...
def get_parent_serial_number(
    child_devinst, child_vid, child_pid, depth=0, last_serial_number=None
):
...
def iterate_comports():
    ...
    if not SetupDiClassGuidsFromName(
        "Ports", PortsGUIDs, ctypes.sizeof(PortsGUIDs), ctypes.byref(ports_guids_size)
    ):
        raise ctypes.WinError()
    ...
    GUIDs = (
        PortsGUIDs[: ports_guids_size.value] + ModemsGUIDs[: modems_guids_size.value]
    )
        ...
        while SetupDiEnumDeviceInfo(g_hdi, index, ctypes.byref(devinfo)):
            ...
            if szHardwareID_str.startswith("USB"):
                ...
                if SetupDiGetDeviceRegistryProperty(
                    ...
                    m = re.finditer(
                        r"USBROOT\((\w+)\)|#USB\((\w+)\)", loc_path_str.value
                    )
                    location = []
                    for g in m:
                        if g.group(1):
