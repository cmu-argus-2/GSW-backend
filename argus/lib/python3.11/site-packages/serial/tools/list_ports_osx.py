...
def get_string_property(device_type, property):
    ...
    key = cf.CFStringCreateWithCString(
        kCFAllocatorDefault, property.encode("utf-8"), kCFStringEncodingUTF8
    )
    ...
    if CFContainer:
        ...
        if output is not None:
            ...
            success = cf.CFStringGetCString(
                CFContainer, ctypes.byref(buffer), io_name_size, kCFStringEncodingUTF8
            )
            if success:
...
def GetParentDeviceByType(device, parent_type):
    ...
    while IOObjectGetClass(device) != parent_type:
        ...
        response = iokit.IORegistryEntryGetParentEntry(
            device, "IOService".encode("utf-8"), ctypes.byref(parent)
        )
    ...
    for service in GetIOServicesByType("IOSerialBSDClient"):
        ...
        if device:
            ...
            if usb_device:
                ...
                locationID = (
                    get_int_property(usb_device, "locationID", kCFNumberSInt32Type)
                    or ""
                )
    ...
    for service in services:
        ...
        if device:
            ...
            if usb_device:
                ...
                info.pid = get_int_property(
                    usb_device, "idProduct", kCFNumberSInt16Type
                )
                info.serial_number = get_string_property(
                    usb_device, kUSBSerialNumberString
                )
                ...
                locationID = get_int_property(
                    usb_device, "locationID", kCFNumberSInt32Type
                )
