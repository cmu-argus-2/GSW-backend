...
from serial.serialutil import (PortNotOpenError, SerialBase, SerialException,
                               SerialTimeoutException, iterbytes, to_bytes)
...
class Serial(SerialBase):
    ...
    def _update_rts_state(self):
        ...
        if self.logger:
            self.logger.info(
                "_update_rts_state({!r}) -> state of CTS".format(self._rts_state)
            )
    ...
    def _update_dtr_state(self):
        ...
        if self.logger:
            self.logger.info(
                "_update_dtr_state({!r}) -> state of DSR".format(self._dtr_state)
            )
