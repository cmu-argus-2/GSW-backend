...
from .types import CMY, CMYK, HLS, HSV, RGB, XYZ, YIQ, YUV, Lab, Luv
...
def matrix_mult(m, n):
    ...
    return (sum(mval * nval for mval, nval in zip(mrow, n)) for mrow in m)


class YUVCoefficients(
    namedtuple(
        "YUVCoefficients",
        ("Wr", "Wg", "Wb", "Umax", "Vmax", "U", "V", "Rv", "Gu", "Gv", "Bu"),
    )
):
    ...
    def __new__(cls, Umax=0.436, Vmax=0.615, **kwargs):
        try:
            ...
            raise TypeError(
                "YUVCoefficients() missing required keyword "
        ...
        Gu = (Wb * (1 - Wb)) / (Umax * Wg)
        Gv = (Wr * (1 - Wr)) / (Vmax * Wg)
        return super(YUVCoefficients, cls).__new__(
            cls, Wr, Wg, Wb, Umax, Vmax, U, V, Rv, Gu, Gv, Bu
        )
...
def html_to_rgb_bytes(html):
    ...
    if html.startswith("#"):
        try:
            ...
            pass
    raise ValueError("{:s} is not a valid HTML color specification".format(html))
...
def xyz_to_lab(x, y, z, white=D65):
    ...
    fx, fy, fz = (
        t ** Fraction(1, 3)
        if t > theta**3
        else t / (3 * theta**2) + Fraction(4, 29)
        for t in (x, y, z)
    )
