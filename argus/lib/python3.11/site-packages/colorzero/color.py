...
from . import deltae, easings, tables, types
    ...
    *directly* manipulated (e.g. by setting the red component).
        ...
        if kwargs:
            try:
                ...
                return {
                    ...
                    frozenset(
                        ("cyan", "magenta", "yellow")
                    ): lambda cyan, magenta, yellow: cls.from_cmy(
                        cyan, magenta, yellow
                    ),
                    frozenset(
                        ("cyan", "magenta", "yellow", "black")
                    ): lambda cyan, magenta, yellow, black: cls.from_cmyk(
                        cyan, magenta, yellow, black
                    ),
                    frozenset(
                        ("hue", "lightness", "saturation")
                    ): lambda hue, lightness, saturation: cls.from_hls(
                        hue, lightness, saturation
                    ),
                    frozenset(
                        ("hue", "saturation", "value")
                    ): lambda hue, saturation, value: cls.from_hsv(
                        hue, saturation, value
                    ),
    ...
    def from_rgb(cls, r, g, b):
        ...
        return super().__new__(
            cls, cv.clamp_float(r), cv.clamp_float(g), cv.clamp_float(b)
        )
    ...
    def __add__(self, other):
        if isinstance(other, types.RGB):
            return Color.from_rgb(self.r + other.r, self.g + other.g, self.b + other.b)
    ...
    def __format__(self, format_spec):
        ...
        if not m:
            raise ValueError("Invalid format {:r} for Color".format(format_spec))
        if m.group("html"):
            return self.html
        elif m.group("css"):
    ...
    def _format_css(self, cssfmt):
        return {
            ...
            "hsl": lambda self: "hsl({0:g}deg, {1:g}%, {2:g}%)".format(
                self.hls.hue.deg, self.hls.saturation * 100, self.hls.lightness * 100
            ),
        }[cssfmt](self)
    ...
    def _format_term(self, back, term):
        if term == "0":
            ...
            if back == "b":
                code = 40
                table = {
                    k: (bold, index) for k, (bold, index) in table.items() if not bold
                }
            else:
                code = 30
            try:
    ...
    def __repr__(self):
        try:
            ...
            raise ValueError("invalid repr_style value: {}".format(Color.repr_style))
    ...
    def gradient(self, other, steps=10, easing=easings.linear):
        ...
        delta = types.RGB(*(other_i - self_i for self_i, other_i in zip(self, other)))
