...
from .exc import DeviceClosed, InputDeviceError, SPIBadChannel
...
class SPIDevice(Device):
    ...
    def __repr__(self):
        try:
            self._check_open()
            return (
                f"<gpiozero.{self.__class__.__name__} object using " f"{self._spi!r}>"
            )
        except DeviceClosed:
...
class MCP3xxx(AnalogInputDevice):
    ...
    def __init__(
        self, channel=0, bits=10, differential=False, max_voltage=3.3, **spi_args
    ):
...
class MCP3xx2(MCP3xxx):
    def _send(self):
        ...
        return self._int_to_words(
            (0b1001 | (not self.differential) << 2 | self.channel << 1)
            << (self.bits + 1)
        )
