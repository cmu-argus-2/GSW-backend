...
from .exc import (BadEventHandler, BadQueueLen, BadWaitTime, CallbackSetToNone,
                  DeviceClosed)
...
class EventsMixin:
    ...
    def active_time(self):
        ...
        if self._active_event.is_set():
            return self.pin_factory.ticks_diff(
                self.pin_factory.ticks(), self._last_changed
            )
        else:
            return None
...
class HoldMixin(EventsMixin):
    ...
    def held_time(self):
        ...
        if self._held_from is not None:
            return self.pin_factory.ticks_diff(
                self.pin_factory.ticks(), self._held_from
            )
        else:
            return None
...
class HoldThread(GPIOThread):
    ...
    def __init__(self, parent):
        super().__init__(target=self.held, args=(weakref.proxy(parent),))
        self.holding = Event()
        self.start()
            ...
            while not self.stopping.is_set():
                if self.holding.wait(0.1):
                    ...
                    while not (
                        ...
                        or parent._inactive_event.wait(parent.hold_time)
                    ):
        ...
        try:
            while not self.stopping.wait(self.sample_wait):
                ...
                if not self.full.is_set() and len(self.queue) >= self.queue.maxlen:
                    self.full.set()
                if (self.partial or self.full.is_set()) and isinstance(
                    self.parent, EventsMixin
                ):
                    self.parent._fire_events(
                        self.parent.pin_factory.ticks(), self.parent.is_active
                    )
        except ReferenceError:
            # Parent is dead; time to die!
            pass
