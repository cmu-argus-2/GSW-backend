...
from .exc import (DeviceClosed, GPIOPinMissing, OutputDeviceBadValue,
                  PWMSoftwareFallback)
...
class PWMOutputDevice(OutputDevice):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            pin, active_high=active_high, initial_value=None, pin_factory=pin_factory
        )
        try:
    ...
    def _blink_device(self, on_time, off_time, fade_in_time, fade_out_time, n, fps=25):
        ...
        sequence = (
            cycle(sequence) if n is None else chain.from_iterable(repeat(sequence, n))
        )
...
class TonalBuzzer(SourceMixin, CompositeDevice):
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"{self._mid_tone!r} is too low for {self._octaves} " f"octaves"
                )
            try:
                self.max_tone.note
            except ValueError:
                raise ValueError(
                    f"{self._mid_tone!r} is too high for {self._octaves} " f"octaves"
                )
            self.value = initial_value
        except:
            self.close()
    ...
    def value(self):
        ...
        if self.pwm_device.pin.frequency is None:
            ...
            return (
                log2(self.pwm_device.pin.frequency / self.mid_tone.frequency)
                / self.octaves
            )
...
class RGBLED(SourceMixin, Device):
    ...
    def blink(
        ...
        self._blink_thread = GPIOThread(
            self._blink_device,
            (on_time, off_time, fade_in_time, fade_out_time, on_color, off_color, n),
        )
        self._blink_thread.start()
        if not background:
            self._blink_thread.join()
    ...
    def __init__(self, forward, backward, *, enable=None, pwm=True, pin_factory=None):
        ...
        if enable is not None:
            devices["enable_device"] = DigitalOutputDevice(
                enable, initial_value=True, pin_factory=pin_factory
            )
...
class PhaseEnableMotor(SourceMixin, CompositeDevice):
    ...
    def value(self):
        ...
        return (
            ...
            if self.phase_device.is_active
            else self.enable_device.value
        )
...
class AngularServo(Servo):
    ...
    def angle(self):
        ...
        if result is None:
            ...
            return round(
                self._angular_range * ((result - self._min_value) / self._value_range)
                + self._min_angle,
                12,
            )
