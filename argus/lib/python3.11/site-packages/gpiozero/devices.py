...
from .exc import (BadPinFactory, CompositeDeviceBadDevice,
                  CompositeDeviceBadName, CompositeDeviceBadOrder,
                  DeviceClosed, GPIODeviceClosed, GPIOPinMissing,
                  NativePinFactoryFallback, PinFactoryFallback)
from .mixins import SharedMixin, ValuesMixin
...
class GPIOBase(metaclass=GPIOMeta):
    ...
    def _check_open(self):
        if self.closed:
            raise DeviceClosed(f"{self.__class__.__name__} is closed or uninitialized")
        ...
        try:
            if self._order is None:
                ...
                for missing_name in set(kwargs.keys()) - set(self._order):
                    raise CompositeDeviceBadOrder(f"{missing_name} missing from _order")
            ...
            for dev in chain(args, kwargs.values()):
                if not isinstance(dev, Device):
                    raise CompositeDeviceBadDevice(f"{dev} doesn't inherit from Device")
