...
def load_segment_font(filename_or_obj, width, height, pins):
    ...
    line_numbers = {
        row_index: line_index for row_index, (line_index, row) in enumerate(rows)
    }
    rows = [row for index, row in rows]
    if len(rows) % height:
        raise ValueError(f"number of definition lines is not divisible by {height}")
    ...
    for n in range(0, len(rows), height):
        cols = [
            col
            for col in zip_longest(*rows[n : n + height], fillvalue=" ")
    ...
    chars = [
        "".join(char for row in rows[y::height] for char in row)[x::width]
    ...
    return {char[0]: tuple(int(char[pos] == on) for pos, on in pins) for char in chars}
...
def load_font_7seg(filename_or_obj):
    ...
    """
    return load_segment_font(
        filename_or_obj,
        width=3,
        height=3,
        pins=[(1, "_"), (5, "|"), (8, "|"), (7, "_"), (6, "|"), (3, "|"), (4, "_")],
    )
