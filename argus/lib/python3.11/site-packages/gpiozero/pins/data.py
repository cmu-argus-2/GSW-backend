...
def gpiof(*names):
    return {
        "gpio"
        if re.match(r"GPIO\d+$", name)
        else "i2c"
        if re.match(r"I2C\d (SDA|SCL)$", name)
        else "spi"
        if re.match(r"SPI\d (SCLK|MOSI|MISO|CE\d)$", name)
        else "uart"
        if re.match(r"UART\d (RXD|TXD|RTS|CTS)$", name)
        else "smi"
        if re.match(r"SMI (name[AD]\d+|SOE / SE|SWE / SRW)$", name)
        else "dpi"
        if re.match(r"DPI (D\d+|PCLK|DE|[HV]SYNC)$", name)
        else "pwm"
        if re.match(r"PWM\d+ \d+$", name)
        else "pcm"
        if re.match(r"PCM (CLK|FS|DIN|DOUT)$", name)
        else "sdio"
        if re.match(r"SD\d+ (CLK|CMD|DAT\d+)$", name)
        else "jtag"
        if re.match(r"JTAG (TDI|TDO|TCK|TMS|RTCK|TRST)$", name)
        else "mii"
        if re.match(r"(RG)?MII ", name)
        else "": name
        for name in names
        if name
    }
...
PI1_GPIO16 = gpiof(
    "GPIO16", "", "SMI SD8", "DPI D11", "UART0 CTS", "SPI1 CE2", "UART1 CTS"
)
PI1_GPIO17 = gpiof(
    "GPIO17", "", "SMI SD9", "DPI D13", "UART0 RTS", "SPI1 CE1", "UART1 RTS"
)
PI1_GPIO18 = gpiof(
    "GPIO18", "PCM CLK", "SMI SD10", "DPI D14", "BSC SDA / MOSI", "SPI1 CE0", "PWM0 0"
)
PI1_GPIO19 = gpiof(
    "GPIO19", "PCM FS", "SMI SD11", "DPI D15", "BSC SCL / SCLK", "SPI1 MISO", "PWM0 1"
)
PI1_GPIO20 = gpiof(
    "GPIO20", "PCM DIN", "SMI SD12", "DPI D16", "BSC MISO", "SPI1 MOSI", "GPCLK0"
)
PI1_GPIO21 = gpiof(
    "GPIO21", "PCM DOUT", "SMI SD13", "DPI D17", "BSC CE", "SPI1 SCLK", "GPCLK1"
)
...
#                   gpio      alt0         alt1             alt2         alt3              alt4                alt5
PI4_GPIO0 = gpiof(
    "GPIO0", "I2C0 SDA", "SMI SA5", "DPI PCLK", "SPI3 CE0", "UART2 TXD", "I2C6 SDA"
)
PI4_GPIO1 = gpiof(
    "GPIO1", "I2C0 SCL", "SMI SA4", "DPI DE", "SPI3 MISO", "UART2 RXD", "I2C6 SCL"
)
PI4_GPIO2 = gpiof(
    "GPIO2", "I2C1 SDA", "SMI SA3", "DPI VSYNC", "SPI3 MOSI", "UART2 CTS", "I2C3 SDA"
)
PI4_GPIO3 = gpiof(
    "GPIO3", "I2C1 SCL", "SMI SA2", "DPI HSYNC", "SPI3 SCLK", "UART2 RTS", "I2C3 SCL"
)
PI4_GPIO4 = gpiof(
    "GPIO4", "GPCLK0", "SMI SA1", "DPI D0", "SPI4 CE0", "UART3 TXD", "I2C3 SDA"
)
PI4_GPIO5 = gpiof(
    "GPIO5", "GPCLK1", "SMI SA0", "DPI D1", "SPI4 MISO", "UART3 RXD", "I2C3 SCL"
)
PI4_GPIO6 = gpiof(
    "GPIO6", "GPCLK2", "SMI SOE / SE", "DPI D2", "SPI4 MOSI", "UART3 CTS", "I2C4 SDA"
)
PI4_GPIO7 = gpiof(
    "GPIO7", "SPI0 CE1", "SMI SWE / SRW", "DPI D3", "SPI4 SCLK", "UART3 RTS", "I2C4 SCL"
)
PI4_GPIO8 = gpiof(
    "GPIO8", "SPI0 CE0", "SMI SD0", "DPI D4", "BSC CE", "UART4 TXD", "I2C4 SDA"
)
PI4_GPIO9 = gpiof(
    "GPIO9", "SPI0 MISO", "SMI SD1", "DPI D5", "BSC MISO", "UART4 RXD", "I2C4 SCL"
)
PI4_GPIO10 = gpiof(
    "GPIO10",
    "SPI0 MOSI",
...
PI4_GPIO12 = gpiof(
    "GPIO12", "PWM0 0", "SMI SD4", "DPI D8", "SPI5 CE0", "UART5 TXD", "I2C5 SDA"
)
PI4_GPIO13 = gpiof(
    "GPIO13", "PWM0 1", "SMI SD5", "DPI D9", "SPI5 MISO", "UART5 RXD", "I2C5 SCL"
)
PI4_GPIO14 = gpiof(
    "GPIO14", "UART0 TXD", "SMI SD6", "DPI D10", "SPI5 MOSI", "UART5 CTS", "UART1 TXD"
)
PI4_GPIO15 = gpiof(
    "GPIO15", "UART0 RXD", "SMI SD7", "DPI D11", "SPI5 SCLK", "UART5 RTS", "UART1 RXD"
)
PI4_GPIO16 = gpiof(
    "GPIO16", "", "SMI SD8", "DPI D12", "UART0 CTS", "SPI1 CE2", "UART1 CTS"
)
PI4_GPIO17 = gpiof(
    "GPIO17", "", "SMI SD9", "DPI D13", "UART0 RTS", "SPI1 CE1", "UART1 RTS"
)
PI4_GPIO18 = gpiof(
    "GPIO18", "PCM CLK", "SMI SD10", "DPI D14", "SPI6 CE0", "SPI1 CE0", "PWM0 0"
)
PI4_GPIO19 = gpiof(
    "GPIO19", "PCM FS", "SMI SD11", "DPI D15", "SPI6 MISO", "SPI1 MISO", "PWM0 1"
)
PI4_GPIO20 = gpiof(
    "GPIO20", "PCM DIN", "SMI SD12", "DPI D16", "SPI6 MOSI", "SPI1 MOSI", "GPCLK0"
)
PI4_GPIO21 = gpiof(
    "GPIO21", "PCM DOUT", "SMI SD13", "DPI D17", "SPI6 SCLK", "SPI1 SCLK", "GPCLK1"
)
PI4_GPIO22 = gpiof(
    "GPIO22", "SD0 CLK", "SMI SD14", "DPI D18", "SD1 CLK", "JTAG TRST", "I2C6 SDA"
)
PI4_GPIO23 = gpiof(
    "GPIO23", "SD0 CMD", "SMI SD15", "DPI D19", "SD1 CMD", "JTAG RTCK", "I2C6 SCL"
)
PI4_GPIO24 = gpiof(
    "GPIO24", "SD0 DAT0", "SMI SD16", "DPI D20", "SD1 DAT0", "JTAG TDO", "SPI3 CE1"
)
PI4_GPIO25 = gpiof(
    "GPIO25", "SD0 DAT1", "SMI SD17", "DPI D21", "SD1 DAT1", "JTAG TCK", "SPI4 CE1"
)
PI4_GPIO26 = gpiof(
    "GPIO26", "SDA DAT2", "", "DPI D22", "SD1 DAT2", "JTAG TDI", "SPI5 CE1"
)
PI4_GPIO27 = gpiof(
    "GPIO27", "SDA DAT3", "", "DPI D23", "SD1 DAT3", "JTAG TMS", "SPI6 CE1"
)
PI4_GPIO28 = gpiof(
    "GPIO28", "I2C0 SDA", "SMI SA5", "PCM CLK", "", "MII RX ERR", "RGMII MDIO"
)
PI4_GPIO29 = gpiof(
    "GPIO29", "I2C0 SCL", "SMI SA4", "PCM FS", "", "MII TX ERR", "RGMII MDC"
)
PI4_GPIO30 = gpiof(
    "GPIO30", "", "SMI SA3", "PCM DIN", "UART0 CTS", "MII CRS", "UART1 CTS"
)
PI4_GPIO31 = gpiof(
    "GPIO31", "", "SMI SA2", "PCM DOUT", "UART0 RTS", "MII COL", "UART1 RTS"
)
PI4_GPIO32 = gpiof(
    "GPIO32", "GPCLK0", "SMI SA1", "", "UART0 TXD", "SD CARD PRES", "UART1 TXD"
)
PI4_GPIO33 = gpiof(
    "GPIO33", "", "SMI SA0", "", "UART0 RXD", "SD CARD WRPROT", "UART1 RXD"
)
PI4_GPIO34 = gpiof(
    "GPIO34", "GPCLK0", "SMI SOE / SE", "", "SD1 CLK", "SD CARD LED", "RGMII IRQ"
)
PI4_GPIO35 = gpiof(
    "GPIO35", "SPI0 CE1", "SMI SWE / SRW", "", "SD1 CMD", "RGMII START STOP"
)
PI4_GPIO36 = gpiof(
    "GPIO36",
    "SPI0 CE0",
...
PI4_GPIO38 = gpiof(
    "GPIO38", "SPI0 MOSI", "SMI SD2", "UART0 RTS", "SD1 DAT2", "RGMII MDC", "MII CRS"
)
PI4_GPIO39 = gpiof(
    "GPIO39", "SPI0 SCLK", "SMI SD3", "UART0 CTS", "SD1 DAT3", "RGMII IRQ", "MII COL"
)
PI4_GPIO40 = gpiof(
    "GPIO40", "PWM1 0", "SMI SD4", "", "SD1 DAT4", "SPI0 MISO", "UART1 TXD"
)
PI4_GPIO41 = gpiof(
    "GPIO41", "PWM1 1", "SMI SD5", "", "SD1 DAT5", "SPI0 MOSI", "UART1 RXD"
)
PI4_GPIO42 = gpiof(
    "GPIO42", "GPCLK1", "SMI SD6", "", "SD1 DAT6", "SPI0 SCLK", "UART1 RTS"
)
PI4_GPIO43 = gpiof(
    "GPIO43", "GPCLK2", "SMI SD7", "", "SD1 DAT7", "SPI0 CE0", "UART1 CTS"
)
PI4_GPIO44 = gpiof(
    "GPIO44", "GPCLK1", "I2C0 SDA", "I2C1 SDA", "", "SPI0 CE1", "SD CARD VOLT"
)
PI4_GPIO45 = gpiof(
    "GPIO45", "PWM0 1", "I2C0 SCL", "I2C1 SCL", "", "SPI0 CE2", "SD CARD PWR0"
)
...
CM4_J2 = (
    ...
    2,
    {
        1: {"": "1-2 DISABLE eMMC BOOT"},
        2: {"": "1-2 DISABLE eMMC BOOT"},
        3: {"": "3-4 WRITE-PROT EEPROM"},
        4: {"": "3-4 WRITE-PROT EEPROM"},
        5: {"": "AIN0 MXL7704"},
        6: {"": "AIN1 MXL7704"},
        7: GND,
        8: {"": "SYNC_IN"},
...
PI_REVISIONS = {
    ...
    0x2: (
        ...
        1,
        1,
        {"P1": REV1_P1, "P2": PI1_P2, "P3": PI1_P3},
        REV1_BOARD,
    ),
    0x3: (
        "B",
    ...
    0x4: (
        ...
        1,
        1,
        {"P1": REV2_P1, "P2": PI1_P2, "P3": PI1_P3, "P5": REV2_P5, "P6": REV2_P6},
        REV2_BOARD,
    ),
    0x5: (
        "B",
