...
from ..exc import (GPIOPinInUse, PinInvalidPin, PinNonPhysical, PinNoPins,
                   PinUnknownPi, SPIBadArgs, SPISoftwareFallback)
from . import BoardInfo, Factory, HeaderInfo, Pin, PinInfo, data
    ...
    def from_revision(cls, revision):
        ...
        if revision & 0x800000:
            ...
            headers = {
                ...
                "B": {"P1": data.REV1_P1, "P2": data.PI1_P2, "P3": data.PI1_P3}
                if pcb_revision == "1.0"
                else {
                    "P1": data.REV2_P1,
                    "P5": data.REV2_P5,
            ...
            board = {
                ...
                "Zero": data.ZERO12_BOARD
                if pcb_revision == "1.2"
                else data.ZERO13_BOARD,
...
class PiFactory(Factory):
    ...
    def spi(self, **spi_args):
        ...
        *port* and *device* with pin numbers will raise
        ...
        if kwargs:
            raise SPIBadArgs(f"unrecognized keyword argument {kwargs.popitem()[0]}")
        try:
        ...
        return self._get_spi_class(shared, hardware=False)(pin_factory=self, **spi_args)
    ...
    def _extract_spi_args(self, **kwargs):
        ...
        kwargs = {key: value for (key, value) in kwargs.items() if key not in spi_args}
        if not spi_args:
            spi_args = pin_defaults
        elif set(spi_args) <= set(pin_defaults):
            spi_args = {
                key: None
                if spi_args.get(key, default) is None
                else self.board_info.to_gpio(spi_args.get(key, default))
                for key, default in pin_defaults.items()
            }
        elif set(spi_args) <= set(dev_defaults):
            spi_args = {
                key: spi_args.get(key, default) for key, default in dev_defaults.items()
            }
            try:
                selected_hw = SPI_HARDWARE_PINS[spi_args["port"]]
            except KeyError:
                raise SPIBadArgs(f"port {spi_args['port']} is not a valid SPI port")
            try:
                selected_hw["select"][spi_args["device"]]
            except IndexError:
                raise SPIBadArgs(
                    f"device must be in the range 0.." f"{len(selected_hw['select'])}"
                )
            # XXX: This is incorrect; assumes port == dev_defaults['port']
            spi_args = {
                key: value
                if key != "select_pin"
                else selected_hw["select"][spi_args["device"]]
