...
from ..exc import (PinFixedPull, PinInvalidEdges, PinInvalidFunction,
                   PinInvalidPull, PinSetInput)
...
def dt_peripheral_reg(node, root=Path("/proc/device-tree")):
    ...
    def _reader(*lengths):
        ...
        offsets = [sum(s.size for s in structs[:i]) for i in range(len(structs))]
...
class GPIOMemory:
    ...
    def __init__(self, soc):
        ...
        try:
            self.reg_fmt = {struct.calcsize(fmt): fmt for fmt in ("@I", "@L")}[4]
        except KeyError:
            ...
            try:
                ...
                for i in range(10):
                    try:
                        ...
                        # triggered mode
                        result = os.open(
                            self.path_value(pin), os.O_RDONLY | os.O_NONBLOCK
                        )
                    except IOError as e:
    ...
    def _run(self, factory, queue):
        ...
        while not self._stop_evt.wait(0):
            ...
            try:
                ...
                if (
                    pin._bounce is None
                    or pin._last_call is None
                    or factory.ticks_diff(ticks, pin._last_call) > pin._bounce
                ):
...
class NativePin(LocalPiPin):
    ...
    def _get_function(self):
        return self.GPIO_FUNCTION_NAMES[
            (self.factory.mem[self._func_offset] >> self._func_shift) & 7
        ]
    ...
    def _set_function(self, value):
        ...
        self.factory.mem[self._func_offset] = self.factory.mem[self._func_offset] & ~(
            7 << self._func_shift
...
class Native2835Pin(NativePin):
    ...
    def _set_pull(self, value):
        ...
        try:
            ...
            raise PinInvalidPull(f'invalid pull direction "{value}" for pin {self!r}')
...
class Native2711Pin(NativePin):
    ...
    def _set_pull(self, value):
        ...
        try:
            ...
            raise PinInvalidPull(f'invalid pull direction "{value}" for pin {self!r}')
        self.factory.mem[self._pull_offset] = self.factory.mem[self._pull_offset] & ~(
            3 << self._pull_shift
