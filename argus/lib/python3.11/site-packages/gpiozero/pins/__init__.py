...
from ..exc import (GPIOPinInUse, PinEdgeDetectUnsupported, PinFixedPull,
                   PinInvalidFunction, PinInvalidPin, PinMultiplePins,
                   PinNoPins, PinPWMUnsupported, PinSetInput,
                   PinSPIUnsupported, PinUnsupported, SPIFixedBitOrder,
                   SPIFixedClockMode, SPIFixedRate, SPIFixedSelect,
                   SPIFixedWordSize)
...
class Factory:
    ...
    * :meth:`spi`
    ...
    def release_pins(self, reserver, *names):
        ...
        """
        pins = (
            info for name in names for header, info in self.board_info.find_pin(name)
        )
        with self._res_lock:
            for pin in pins:
                self._reservations[pin] = [
                    ref
                    for ref in self._reservations[pin]
    ...
    def release_all(self, reserver):
        ...
        self.release_pins(reserver, *(pin.name for pin in self._reservations))
...
class Pin:
    ...
    * :meth:`_set_when_changed`
    ...
    def _set_pull(self, value):
        raise PinFixedPull(f"Cannot change pull-up on pin {self!r}")  # pragma: no cover
...
class SPI(Device):
    ...
    * :meth:`_set_bits_per_word`
    ...
    def _set_rate(self, value):
        raise SPIFixedRate(f"rate cannot be changed on {self!r}")  # pragma: no cover
        ...
        return "".join(
            (
                ...
                (
                    " "
                    if pin is None
                    else self._pin_style(pin, style)
                    + (
                        "1"
                        if pin.number == 1
                        else "2"
                        if pin.number == 2 and self.rows == self.columns
                        else "o"
                    )
                ),
                style("reset"),
...
class BoardInfo(
    ...
    def physical_pins(self, function):
        ...
        return {(header.name, pin.number) for header, pin in self.find_pin(function)}
    ...
    def __repr__(self):
        fields = ", ".join(
            f"{name}=..." if name in ("headers", "board") else f"{name}={value!r}"
    ...
    def __format__(self, format_spec):
        ...
        if content == "full":
            ...
            kw = self._asdict()
            kw.update({name: header for name, header in self.headers.items()})
