...
from ..exc import (DeviceClosed, PinFixedPull, PinInvalidBounce,
                   PinInvalidFunction, PinInvalidPull, PinInvalidState,
                   PinPWMFixedValue, PinSetInput, SPIBadArgs,
                   SPIInvalidClockMode)
...
class PiGPIOPin(PiPin):
    ...
    def _get_function(self):
        return self.GPIO_FUNCTION_NAMES[self.factory.connection.get_mode(self._number)]

    def _set_function(self, value):
        if value != "input":
            self._pull = "floating"
        try:
            self.factory.connection.set_mode(self._number, self.GPIO_FUNCTIONS[value])
        except KeyError:
            raise PinInvalidFunction(f'invalid function "{value}" for pin {self!r}')

    def _get_state(self):
        if self._pwm:
            return self.factory.connection.get_PWM_dutycycle(
                self._number
    ...
    def _set_state(self, value):
        if self._pwm:
            try:
                ...
                raise PinInvalidState(f'invalid state "{value}" for pin {self!r}')
    ...
    def _set_bounce(self, value):
        ...
        self.factory.connection.set_glitch_filter(self._number, int(value * 1000000))
...
class PiGPIOHardwareSPI(SPI):
    ...
    def __init__(self, clock_pin, mosi_pin, miso_pin, select_pin, pin_factory):
        port, device = spi_port_device(clock_pin, mosi_pin, miso_pin, select_pin)
        self._port = port
        self._device = device
        self._handle = None
    ...
    def _conflicts_with(self, other):
        return not (
            isinstance(other, PiGPIOHardwareSPI)
            and (self.pin_factory.host, self._port, self._device)
    ...
    def _set_select_high(self, value):
        self._check_open()
        self.pin_factory.connection.spi_close(self._handle)
        self._spi_flags = (self._spi_flags & ~0x1C) | (
            bool(value) << (2 + self._device)
        )
        self._handle = self.pin_factory.connection.spi_open(
            self._device, self._baud, self._spi_flags
        )
    ...
    def _set_lsb_first(self, value):
        if self._port:
            ...
            self.pin_factory.connection.spi_close(self._handle)
            self._spi_flags = (
                (self._spi_flags & ~0xC000) | (bool(value) << 14) | (bool(value) << 15)
            )
            self._handle = self.pin_factory.connection.spi_open(
                self._device, self._baud, self._spi_flags
            )
...
class PiGPIOSoftwareSPI(SPI):
    ...
    def __init__(self, clock_pin, mosi_pin, miso_pin, select_pin, pin_factory):
        ...
        try:
            self.pin_factory.connection.bb_spi_open(
                select_pin, miso_pin, mosi_pin, clock_pin, self._baud, self._spi_flags
            )
    ...
    def _conflicts_with(self, other):
        return not (
            isinstance(other, PiGPIOSoftwareSPI)
            and (self._select_pin) != (other._select_pin)
        )
    ...
    def transfer(self, data):
        self._check_open()
        count, data = self.pin_factory.connection.bb_spi_xfer(self._select_pin, data)
        if count < 0:
            raise IOError(f"SPI transfer error {count}")
