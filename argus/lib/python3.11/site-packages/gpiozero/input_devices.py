...
from .devices import CompositeDevice, GPIODevice
from .exc import (DeviceClosed, DistanceSensorNoEcho, InputDeviceError,
                  PinInvalidState, PWMSoftwareFallback)
from .mixins import EventsMixin, GPIOQueue, HoldMixin, event
...
class DigitalInputDevice(EventsMixin, InputDevice):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            pin, pull_up=pull_up, active_state=active_state, pin_factory=pin_factory
        )
        try:
...
class SmoothedInputDevice(EventsMixin, InputDevice):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            self._queue = GPIOQueue(
                self, queue_len, sample_wait, partial, average, ignore
            )
...
class RotaryEncoder(EventsMixin, CompositeDevice):
    ...
    def _change_state(self, ticks, edge):
        ...
        if new_state == "+1":
            self._steps = (
                self._steps + 1
                if not self._max_steps or self._steps < self._max_steps
