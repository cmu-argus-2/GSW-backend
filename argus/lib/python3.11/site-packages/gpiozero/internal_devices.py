...
from .exc import DeviceClosed, ThresholdOutOfRange
...
class PolledInternalDevice(InternalDevice):
    ...
    def wait_for_active(self, timeout=None):
        ...
        try:
            ...
            self._start_stop_events(self.when_activated or self.when_deactivated)
        ...
        with io.open(os.devnull, "wb") as devnull:
            try:
                subprocess.check_call(
                    ["ping", "-c1", self.host], stdout=devnull, stderr=devnull
                )
...
class LoadAverage(PolledInternalDevice):
    ...
    def __init__(
        self,
        load_average_file="/proc/loadavg",
        *,
        min_load_average=0.0,
        ...
        if min_load_average >= max_load_average:
            raise ValueError("max_load_average must be greater than min_load_average")
...
class TimeOfDay(PolledInternalDevice):
    ...
    *start_time* and *end_time* (inclusive) which are :class:`~datetime.time`
    ...
    def __init__(
        self, start_time, end_time, *, utc=True, event_delay=5.0, pin_factory=None
    ):
...
class DiskUsage(PolledInternalDevice):
    ...
    def __init__(
        self, filesystem="/", *, threshold=90.0, event_delay=30.0, pin_factory=None
    ):
        super().__init__(event_delay=event_delay, pin_factory=pin_factory)
        os.statvfs(filesystem)
        if not 0 <= threshold <= 100:
            warnings.warn(
                ThresholdOutOfRange("threshold is outside of the range (0, 100)")
            )
