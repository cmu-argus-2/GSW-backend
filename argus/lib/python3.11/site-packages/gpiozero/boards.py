...
from itertools import chain, cycle, repeat, tee
from pprint import pformat
from threading import Lock
from time import sleep

from .devices import CompositeDevice, Device
from .exc import (BadWaitTime, CompositeDeviceBadDevice, DeviceClosed,
                  EnergenieBadSocket, EnergenieSocketMissing, GPIOPinMissing,
                  OutputDeviceBadValue, PinInvalidPin)
from .fonts import load_font_7seg, load_font_14seg
from .input_devices import Button
from .mixins import HoldMixin, SharedMixin, SourceMixin, event
from .output_devices import (LED, PWMLED, RGBLED, Buzzer, Motor, OutputDevice,
                             PhaseEnableMotor, TonalBuzzer)
...
class LEDBoard(LEDCollection):
    ...
    def blink(
        ...
        self._blink_thread = GPIOThread(
            self._blink_device, (on_time, off_time, fade_in_time, fade_out_time, n)
        )
...
class LEDBarGraph(LEDCollection):
    ...
    def __init__(
        self, *pins, pwm=False, active_high=True, initial_value=0.0, pin_factory=None
    ):
...
class LEDCharFont(MutableMapping):
    ...
    *not* change the state of the LEDs in the display (though it may change
    ...
    def __init__(self, font):
        super().__init__()
        self._map = {
            char: tuple(int(bool(pin)) for pin in pins) for char, pins in font.items()
        }
        self._refresh_rmap()
    ...
    def __init__(
        ...
        *pins,
        ...
        if not 1 < len(pins) <= 26:
            raise PinInvalidPin("Must have between 2 and 26 LEDs in LEDCharDisplay")
        for pin in pins:
            if isinstance(pin, LEDCollection):
                raise PinInvalidPin("Cannot use LEDCollection in LEDCharDisplay")
        ...
        if font is None:
            if len(pins) == 7:
                ...
                with resources.files("gpiozero.fonts").joinpath(
                    "14seg.txt"
                ).open() as f:
...
class LEDMultiCharDisplay(CompositeOutputDevice):
    ...
    *active_high* parameter. Finally, *initial_value* defaults to a tuple of
    ...
    def __init__(
        self, char, *pins, active_high=True, initial_value=None, pin_factory=None
    ):
        ...
        super().__init__(plex=plex, char=char, pin_factory=pin_factory)
        self.value = initial_value
    ...
    def __init__(
        ...
        *,
        ...
        if amber is not None and yellow is not None:
            raise OutputDeviceBadValue("Only one of amber or yellow can be specified")
        ...
        if not all(p is not None for p in devices.values()):
            raise GPIOPinMissing(f'{", ".join(devices.keys())} pins must be provided')
        super().__init__(
            pwm=pwm,
            initial_value=initial_value,
            _order=devices.keys(),
...
class PiStop(TrafficLights):
    ...
    def __init__(
        self, location=None, *, pwm=False, initial_value=False, pin_factory=None
    ):
        ...
        super().__init__(
            *gpios, pwm=pwm, initial_value=initial_value, pin_factory=pin_factory
        )
...
class StatusZero(LEDBoard):
    ...
    def __init__(self, *labels, pwm=False, initial_value=False, pin_factory=None):
        pins = (
            ("BOARD11", "BOARD7"),
            ("BOARD15", "BOARD13"),
            ("BOARD21", "BOARD19"),
        ...
        if len(labels) == 0:
            ...
            raise ValueError(f"StatusZero doesn't support more than {len(pins)} labels")
...
class TrafficHat(CompositeOutputDevice):
    ...
    def __init__(self, *, pwm=False, pin_factory=None):
        super().__init__(
            lights=TrafficLights(
                "BOARD18", "BOARD16", "BOARD15", pwm=pwm, pin_factory=pin_factory
            ),
...
class Robot(SourceMixin, CompositeDevice):
    ...
    def forward(self, speed=1, *, curve_left=0, curve_right=0):
        ...
        if curve_left != 0 and curve_right != 0:
            raise ValueError(
                "curve_left and curve_right can't be used at " "the same time"
            )
...
class PololuDRV8835Robot(Robot):
    ...
    def __init__(self, *, pwm=True, pin_factory=None):
        super().__init__(
            left=PhaseEnableMotor(
                "BOARD29", "BOARD32", pwm=pwm, pin_factory=pin_factory
            ),
            right=PhaseEnableMotor(
                "BOARD31", "BOARD33", pwm=pwm, pin_factory=pin_factory
            ),
            pin_factory=pin_factory,
        )
...
class Pibrella(CompositeOutputDevice):
    ...
    def __init__(self, *, pwm=False, pin_factory=None):
        ...
        self.inputs = InputPins(a="BOARD21", b="BOARD26", c="BOARD24", d="BOARD19")
        self.outputs = OutputPins(e="BOARD15", f="BOARD16", g="BOARD18", h="BOARD22")
