...
class PintestTool(CliTool):
    ...
    def get_pins(self, board, include, exclude):
        if not include:
            ...
            pins = {pin for name in include for header, pin in board.find_pin(name)}
        skip = {pin for name in exclude for header, pin in board.find_pin(name)}
        pins -= skip
        for pin in pins:
        ...
        with Device.pin_factory.pin(pin_info.name) as pin:
            ...
            try:
                ...
                if pin_info.pull != "up":
                    pin.pull = "down"
                    if pin.state != 0:
                        raise ValueError(f"Pull down on {pin_info.function} failed")
                if pin_info.pull != "down":
                    pin.pull = "up"
                    if pin.state != 1:
                        raise ValueError(f"Pull up on {pin_info.function} failed")
                if pin_info.pull == "":
                    pin.pull = "floating"
            finally:
